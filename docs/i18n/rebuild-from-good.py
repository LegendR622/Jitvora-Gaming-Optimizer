#!/usr/bin/env python3
"""Rebuild locale tables from known-good git commit, then apply v1.9.6 + EN."""
...
EN = {
    ...
    "meta.description": "Redline Gaming Optimizer — premium Windows gaming optimizer for FPS tuning, PC cleaner, driver check and network tools. Safe Windows tweaks. Free download v1.9.6 for Windows 10/11.",
}
...
def main() -> None:
    ...
    for lang, table in good.items():
        merged = dict(table)
        for k, v in mock.get(lang, {}).items():
            if k.startswith("hero.mock."):
                merged[k] = v
        out[lang] = {k: bump_version(merged[k]) for k in keys if k in merged}

    out["en"] = EN.copy()
    # keep current German if present and cleaner
    cur = json.loads(FULL.read_text(encoding="utf-8-sig")) if FULL.exists() else {}
    if cur.get("de"):
        out["de"] = {k: bump_version(cur["de"][k]) for k in keys if k in cur["de"]}

    FULL.write_text(json.dumps(out, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(f"Rebuilt {len(out)} locales from {GOOD_REF}")
