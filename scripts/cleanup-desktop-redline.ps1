...
Get-ChildItem $desktop -Filter "*.exe" -File -EA SilentlyContinue |
    Where-Object {
        ...
        ($_.Name -match '^Redline_Gaming_Optimizer_Setup' -and $_.Name -ne 'Redline_Gaming_Optimizer_Setup_v9.21.exe')
    } | ForEach-Object {
...
$setupSrc = Join-Path $root "dist\Redline_Gaming_Optimizer_Setup_v9.21.exe"
$setupDesk = Join-Path $desktop "Redline_Gaming_Optimizer_Setup_v9.21.exe"
